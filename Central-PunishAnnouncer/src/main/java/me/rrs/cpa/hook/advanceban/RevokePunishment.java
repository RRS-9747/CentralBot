...
public class RevokePunishment implements Listener {

    final CentralBotAPI api = new CentralBotAPI(CentralPunishAnnouncer.getInstance());

    @EventHandler
    public void onRemovePunish(RevokePunishmentEvent event) {
        ...
        switch (punish.getType().getName().toUpperCase()) {
            ...
            case "TEMPIPBAN":
            case "TIPBAN":
                title = punish.getType().isIpOrientated() ? "IP Ban Revoked" : "Ban Revoked";
